...
class CometModel(ptl.LightningModule, metaclass=abc.ABCMeta):
    ...
    def validation_epoch_end(self, *args, **kwargs) -> None:
        ...
        values = {
            'val_loss': (kendall_metrics.mean().mean()+pearson_metrics.mean().mean()+spearman_metrics.mean().mean())/3,
    ...
    def setup(self, stage) -> None:
        ...
        if stage in (None, "fit"):
            ...
            data = self.read_csv(self.hparams.training_data_path, self.hparams.validation_data_path)

            self.training_dataset = data['training_data']
            self.validation_dataset = data['validation_data']


            self.epoch_total_steps = len(self.training_dataset) // (
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        dataloader_sum_doc = DataLoader(
              dataset=self.training_dataset['sum_doc'],
              sampler=RandomSampler(self.training_dataset['sum_doc']),
        ...
        dataloader_sum_ref = DataLoader(
              dataset=self.training_dataset['sum_ref'],
              sampler=RandomSampler(self.training_dataset['sum_ref']),
              batch_size=self.hparams.batch_size,
              collate_fn=self.prepare_sample,
              num_workers=min(4, multiprocessing.cpu_count()),
        )
